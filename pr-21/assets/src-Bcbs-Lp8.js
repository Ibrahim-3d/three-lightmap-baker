import{B as e,Bn as t,Et as n,Ft as r,L as i,Mt as a,T as o,Vn as s,br as c,qt as l,vt as u}from"./three.core-DCTFLKPF.js";import{n as d,t as f}from"./preprocess-BZ-4Bhl6.js";var p=`/**
...
`,h=class extends s{constructor(t,n){d();let r={tTriangleTexture:{value:t.triangleTexture},tAABBTexture:{value:t.aabbTexture},tAlbedoArray:{value:t.albedoArray},tPreviousTexture:{value:null},tBlueNoiseTexture:{value:null},uSampleCounter:{value:1},uFrameCounter:{value:1},uEPS_intersect:{value:.001},uRandomVec2:{value:{x:0,y:0}},uHasSkyTexture:{value:!1},tHDRTexture:{value:null},uSkyLightIntensity:{value:1},tLightTexture:{value:n},uNumPTLights:{value:0},uResolution:{value:{x:1,y:1}},uCameraMatrix:{value:null},uCameraIsMoving:{value:!1},uSceneIsDynamic:{value:!1},uUseOrthographicCamera:{value:!1},uApertureSize:{value:0},uFocusDistance:{value:100},uULen:{value:1},uVLen:{value:1},uTime:{value:0},uPreviousSampleCount:{value:1}};super({vertexShader:p,fragmentShader:f(m),uniforms:r,glslVersion:e}),this.customProgramCacheKey=()=>`pt-bake-v1`}setPreviousTexture(e){let t=this.uniforms.tPreviousTexture;if(!t)throw Error(`[pt-baker] missing tPreviousTexture uniform`);t.value=e?.texture??null}};function g(){let e=new o(new Float32Array(256),64,1,l,i);return e.colorSpace=``,e.minFilter=e.magFilter=n,e.generateMipmaps=!1,e.needsUpdate=!0,e}function _(e){let t=new c(e,e,{type:i,format:l,minFilter:n,magFilter:n,generateMipmaps:!1,depthBuffer:!1});return t.texture.colorSpace=``,t.texture.flipY=!1,t}var v=`
...
`;function b(){return new Promise(e=>setTimeout(e,0))}function x(e,t){let n=e.uniforms[t];if(!n)throw Error(`[pt-baker] missing ${t} uniform`);return n}var S=class{ownedLightTex=null;async bake(e,n,i,o={}){let c=o.size??1024,l=o.samples??128,d=o.yieldEvery??4,f=o.numLights??0,p=o.lightTex??(this.ownedLightTex??=g()),m=_(c),S=_(c),C=_(c),w=new h(i,p);x(w,`uNumPTLights`).value=f,x(w,`uSkyLightIntensity`).value=o.skyIntensity??1;let T=new s({vertexShader:v,fragmentShader:y,uniforms:{tAccum:{value:null},uSampleCount:{value:l}},glslVersion:`300 es`}),E=new Map;for(let e of n)E.set(e,e.material);let D=new a(-1,1,1,-1,0,1),O=new t,k=new u(new r(2,2),T);O.add(k);let A=m,j=S;try{for(let r=0;r<l;r++){x(w,`uSampleCounter`).value=r+1,x(w,`uFrameCounter`).value=r+1,x(w,`uRandomVec2`).value={x:Math.random(),y:Math.random()},w.setPreviousTexture(A);for(let e of n)e.material=w;e.setRenderTarget(j),e.clear();for(let r of n){let n=new t;n.add(r),e.render(n,D),r.parent===n&&n.remove(r)}[A,j]=[j,A],o.onProgress?.((r+1)/l),(r+1)%d===0&&await b()}for(let e of n){let t=E.get(e);t&&(e.material=t)}x(T,`tAccum`).value=A.texture,x(T,`uSampleCount`).value=l,e.setRenderTarget(C),e.clear(),e.render(O,D)}finally{e.setRenderTarget(null)}return{texture:C,samples:l,dispose:()=>{w.dispose(),T.dispose(),m.dispose(),S.dispose(),C.dispose(),this.ownedLightTex?.dispose()}}}dispose(){this.ownedLightTex?.dispose()}};export{S as PTBaker};