...
export function buildBVHScene(scene: Scene): BVHSceneData {
  ...
  for (const mesh of meshes) {
    ...
    if (!nonIndexed.attributes['position']) { geo.dispose(); continue; }
    ...
    if (nonIndexed.groups.length > 0) {
      for (const group of nonIndexed.groups) {
        ...
        materialDescs.push(_matDesc(mat instanceof MeshStandardMaterial ? mat : null, albedoTextures));
  ...
  if (totalTris > MAX_TRIANGLES) {
    console.warn(`[PTSceneBuilder] Scene has ${totalTris} triangles — exceeds 2048² limit of ${MAX_TRIANGLES}. Extra triangles will be ignored.`);
  }
  ...
  for (let i = 0; i < N; i++) {
    ...
    const vp0x = posAttr.getX(i * 3 + 0), vp0y = posAttr.getY(i * 3 + 0), vp0z = posAttr.getZ(i * 3 + 0);
    const vp1x = posAttr.getX(i * 3 + 1), vp1y = posAttr.getY(i * 3 + 1), vp1z = posAttr.getZ(i * 3 + 1);
    const vp2x = posAttr.getX(i * 3 + 2), vp2y = posAttr.getY(i * 3 + 2), vp2z = posAttr.getZ(i * 3 + 2);

    // Vertex normals (9 components, normalized)
    let vn0x = 0, vn0y = 1, vn0z = 0;
    let vn1x = 0, vn1y = 1, vn1z = 0;
    let vn2x = 0, vn2y = 1, vn2z = 0;
    if (nrmAttr) {
      vn0x = nrmAttr.getX(i * 3 + 0); vn0y = nrmAttr.getY(i * 3 + 0); vn0z = nrmAttr.getZ(i * 3 + 0);
      vn1x = nrmAttr.getX(i * 3 + 1); vn1y = nrmAttr.getY(i * 3 + 1); vn1z = nrmAttr.getZ(i * 3 + 1);
      vn2x = nrmAttr.getX(i * 3 + 2); vn2y = nrmAttr.getY(i * 3 + 2); vn2z = nrmAttr.getZ(i * 3 + 2);
    }

    // UV coordinates (6 components, default -1 = no UVs)
    let vt0x = -1, vt0y = -1, vt1x = -1, vt1y = -1, vt2x = -1, vt2y = -1;
    if (uvAttr) {
      vt0x = uvAttr.getX(i * 3 + 0); vt0y = uvAttr.getY(i * 3 + 0);
      vt1x = uvAttr.getX(i * 3 + 1); vt1y = uvAttr.getY(i * 3 + 1);
      vt2x = uvAttr.getX(i * 3 + 2); vt2y = uvAttr.getY(i * 3 + 2);
    }

    // Material assignment
    while (materialNumber < triangleMaterialMarkers.length - 1 && i >= triangleMaterialMarkers[materialNumber]!)
      materialNumber++;
    const md = materialDescs[materialNumber] ?? { type: MAT_PBR, r: 0.8, g: 0.8, b: 0.8, opacity: 1, texID: -1, roughness: 0.8, metalness: 0.0, uvTransform: null };
    ...
    if (uvAttr && md.uvTransform) {
      ...
      const u0=vt0x, v0=vt0y;  const u1=vt1x, v1=vt1y;  const u2=vt2x, v2=vt2y;
      vt0x=m00*u0+m01*v0+m02; vt0y=m10*u0+m11*v0+m12;
      vt1x=m00*u1+m01*v1+m02; vt1y=m10*u1+m11*v1+m12;
      vt2x=m00*u2+m01*v2+m02; vt2y=m10*u2+m11*v2+m12;
    }

    // Pack into triangle_array (32 floats at stride 32*i)
    const base = 32 * i;
    triangle_array[base +  0] = vp0x; triangle_array[base +  1] = vp0y; triangle_array[base +  2] = vp0z; triangle_array[base +  3] = vp1x;
    triangle_array[base +  4] = vp1y; triangle_array[base +  5] = vp1z; triangle_array[base +  6] = vp2x; triangle_array[base +  7] = vp2y;
    triangle_array[base +  8] = vp2z; triangle_array[base +  9] = vn0x; triangle_array[base + 10] = vn0y; triangle_array[base + 11] = vn0z;
    triangle_array[base + 12] = vn1x; triangle_array[base + 13] = vn1y; triangle_array[base + 14] = vn1z; triangle_array[base + 15] = vn2x;
    triangle_array[base + 16] = vn2y; triangle_array[base + 17] = vn2z; triangle_array[base + 18] = vt0x; triangle_array[base + 19] = vt0y;
    triangle_array[base + 20] = vt1x; triangle_array[base + 21] = vt1y; triangle_array[base + 22] = vt2x; triangle_array[base + 23] = vt2y;
    triangle_array[base + 24] = md.type; triangle_array[base + 25] = md.r; triangle_array[base + 26] = md.g; triangle_array[base + 27] = md.b;
    triangle_array[base + 28] = md.texID; triangle_array[base + 29] = md.opacity; triangle_array[base + 30] = md.roughness; triangle_array[base + 31] = md.metalness;
    ...
    const ab = 9 * i;
    aabb_array[ab + 0] = minX; aabb_array[ab + 1] = minY; aabb_array[ab + 2] = minZ;
    aabb_array[ab + 3] = maxX; aabb_array[ab + 4] = maxY; aabb_array[ab + 5] = maxZ;
  ...
  const triangleTexture = new DataTexture(triangle_array, TEX_SIZE, TEX_SIZE, RGBAFormat, FloatType);
...
function _getUVTransform(tex: Texture | null): MatDesc['uvTransform'] {
  ...
  if (Math.abs(m00-1)<1e-5 && Math.abs(m10)<1e-5 && Math.abs(m01)<1e-5 && Math.abs(m11-1)<1e-5 && Math.abs(m02)<1e-5 && Math.abs(m12)<1e-5)
    return null;
  return [m00, m10, m01, m11, m02, m12] as const;
}

function _emptyScene(): BVHSceneData {
  const empty = new DataTexture(new Float32Array(TEX_SIZE * TEX_SIZE * 4), TEX_SIZE, TEX_SIZE, RGBAFormat, FloatType);
  empty.colorSpace = NoColorSpace;
  empty.flipY = false;
  empty.generateMipmaps = false;
  return { triangleTexture: empty, aabbTexture: empty.clone(), albedoTextures: [], triangleCount: 0 };
}

interface MatDesc { type: number; r: number; g: number; b: number; opacity: number; texID: number; roughness: number; metalness: number;
...
function _matDesc(mat: MeshStandardMaterial | null, albedoTextures: Texture[]): MatDesc {
  if (!mat) return { type: MAT_PBR, r: 0.8, g: 0.8, b: 0.8, opacity: 1, texID: -1, roughness: 0.8, metalness: 0.0, uvTransform: null };
  ...
  if (mat.emissiveIntensity > 0 && (mat.emissive.r + mat.emissive.g + mat.emissive.b) > 0.001) {
    ...
    return { type: MAT_LIGHT, r: e.r * ei, g: e.g * ei, b: e.b * ei, opacity: 1, texID: -1, roughness: 0, metalness: 0, uvTransform: null };
  }
  ...
  if (isGlass) {
    ...
    return { type: MAT_REFR, r: mat.color.r, g: mat.color.g, b: mat.color.b, opacity: mat.opacity, texID, roughness: 0, metalness: 0, uvTransform };
  }
