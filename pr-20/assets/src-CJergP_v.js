...
`;function b(){return new Promise(e=>setTimeout(e,0))}var x=class{ownedLightTex=null;async bake(e,n,i,o={}){let c=o.size??1024,l=o.samples??128,d=o.yieldEvery??4,f=o.numLights??0,p=o.lightTex??(this.ownedLightTex??=g()),m=_(c),x=_(c),S=_(c),C=new h(i,p);C.uniforms.uNumPTLights.value=f,C.uniforms.uSkyLightIntensity.value=o.skyIntensity??1;let w=new s({vertexShader:v,fragmentShader:y,uniforms:{tAccum:{value:null},uSampleCount:{value:l}},glslVersion:`300 es`}),T=new Map;for(let e of n)T.set(e,e.material);let E=new a(-1,1,1,-1,0,1),D=new t,O=new u(new r(2,2),w);D.add(O);let k=m,A=x;try{for(let r=0;r<l;r++){C.uniforms.uSampleCounter.value=r+1,C.uniforms.uFrameCounter.value=r+1,C.uniforms.uRandomVec2.value={x:Math.random(),y:Math.random()},C.setPreviousTexture(k);for(let e of n)e.material=C;e.setRenderTarget(A),e.clear();for(let r of n){let n=new t;n.add(r),e.render(n,E),r.parent===n&&n.remove(r)}[k,A]=[A,k],o.onProgress?.((r+1)/l),(r+1)%d===0&&await b()}for(let e of n){let t=T.get(e);t&&(e.material=t)}w.uniforms.tAccum.value=k.texture,w.uniforms.uSampleCount.value=l,e.setRenderTarget(S),e.clear(),e.render(D,E)}finally{e.setRenderTarget(null)}return{texture:S,samples:l,dispose:()=>{C.dispose(),w.dispose(),m.dispose(),x.dispose(),S.dispose(),this.ownedLightTex?.dispose()}}}dispose(){this.ownedLightTex?.dispose()}};export{x as PTBaker};