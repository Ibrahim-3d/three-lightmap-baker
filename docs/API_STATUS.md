...
- Material-albedo-aware source diffuse projection, using the position atlas to
  retain each sample's source mesh identity.
...
- Deterministic single-GPU-worker regression suite covering all 37 browser tests.

### Locally validated on 2026-08-10

- TypeScript source and examples compile.
- The complete 37-test Playwright suite passes sequentially without retries.
...
- Probe source albedo currently supports one solid `material.color` per mesh.
  ...
  solid-color static contributors.
