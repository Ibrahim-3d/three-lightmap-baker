...
export type CompositeResult = {
    ...
    refresh: (uniforms?: Partial<{ directIntensity: number; giIntensity: number; aoEnabled: boolean }>) => void;
    dispose: () => void;
};
...
export const runComposite = (
    ...
    const refresh = (overrides?: Partial<{ directIntensity: number; giIntensity: number; aoEnabled: boolean }>) => {
        if (overrides?.directIntensity !== undefined) mat.uniforms.directIntensity.value = overrides.directIntensity;
        if (overrides?.giIntensity     !== undefined) mat.uniforms.giIntensity.value     = overrides.giIntensity;
