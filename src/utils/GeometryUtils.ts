import { BufferAttribute, BufferGeometry, Material, Mesh, MeshBasicMaterial, MeshStandardMaterial } from "three";
...
export const mergeGeometry = (meshes: Mesh[]): BufferGeometry => {
	...
	return mergeGeometries(prepped) as BufferGeometry;
};
...
const readMaterialColors = (material: Material | Material[]): MatColors => {
	...
	if (Array.isArray(material)) {
		console.warn("[baker] material array detected; using slot 0 only — per-face material groups not yet supported");
	...
	if ("emissive" in m && m.emissive) {
		...
		return {
			aR: m.color.r, aG: m.color.g, aB: m.color.b,
	...
	console.warn("[baker] material has no .color (likely ShaderMaterial); defaulting to white albedo");
 ...
 * mesh.
 */
export const extractPerTriangleMaterials = (merged: BufferGeometry, meshes: Mesh[]): PerTriangleMaterials => {
	...
	if (!meshIdxAttr) {
		throw new Error("[baker] merged geometry is missing 'meshIndex' attribute — did mergeGeometry skip the per-vertex tag?");
	}
	...
	for (let tri = 0; tri < totalTriangles; tri++) {
		...
		const o = tri * 3;
		albedo[o] = c.aR; albedo[o + 1] = c.aG; albedo[o + 2] = c.aB;
		emissive[o] = c.eR; emissive[o + 1] = c.eG; emissive[o + 2] = c.eB;
	}
