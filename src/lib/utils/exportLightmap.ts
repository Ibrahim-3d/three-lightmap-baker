...
function renderToRT(renderer: WebGLRenderer, source: Texture, resolution: number): WebGLRenderTarget {
...
export async function exportPNG(
    ...
    await new Promise<void>((resolve, reject) => {
        canvas.toBlob((blob) => {
            if (!blob) { reject(new Error('exportPNG: toBlob returned null')); return; }
...
export function exportRaw(
    ...
    renderer.readRenderTargetPixels(rt, 0, 0, resolution, resolution, float);
    rt.dispose();
    triggerDownload(new Blob([float.buffer], { type: 'application/octet-stream' }), ensureExt(filename, 'bin'));
}
...
export async function exportLightmap(
    ...
    switch (format) {
        case 'png': await exportPNG(renderer, source, resolution, filename); return;
        case 'exr': exportEXR(renderer, source, resolution, filename); return;
        case 'bin': exportRaw(renderer, source, resolution, filename); return;
    }
}
