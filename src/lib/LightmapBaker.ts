...
export class LightmapBakeResult {
     ...
     */
    async export(pathOrName: string = 'lightmap', opts: { format?: ExportFormat } = {}): Promise<void> {
...
export class LightmapBaker {
    private opts: Required<Omit<LightmapBakerOptions, 'light' | 'gi' | 'ao' | 'refinementOptions'>> & {
    ...
    constructor(public readonly renderer: WebGLRenderer, opts: LightmapBakerOptions = {}) {
        if ((opts.bounces ?? 1) > 1) {
            console.warn(
        ...
        this.opts = {
            ...
            refinementOptions: { ...DEFAULT_REFINEMENT, ...(opts.refinementOptions ?? {}), denoiseEnabled: opts.denoise ?? DEFAULT_REFINEMENT.denoiseEnabled },
        };
    }
    ...
    async bake(scene: Scene | Object3D, hooks: BakeHooks = {}): Promise<LightmapBakeResult> {
        ...
        if (!meshes.length) throw new Error('[baker] no bake-eligible meshes in scene (need Mesh + MeshStandardMaterial-like)');
        scene.updateMatrixWorld(true);

        const checkAbort = () => { if (hooks.signal?.aborted) throw new Error('[baker] aborted'); };
        ...
        const lightmapper = await generateLightmapper(
            this.renderer, atlas.positionTexture, atlas.normalTexture, bvh, raycastOpts,
        );

        await new Promise<void>((resolve, reject) => {
            const tick = () => {
                if (hooks.signal?.aborted) { reject(new Error('[baker] aborted')); return; }
        ...
        const stats: BakeStats = {
            ...
            raysTraced: this.opts.samples * this.opts.castsPerFrame * this.opts.resolution * this.opts.resolution,
            duration: {
                uvUnwrap: tUV1 - tUV0,
                geometry: tG1 - tG0,
                bake:     tB1 - tB0,
        ...
        return new LightmapBakeResult(
            this.renderer, meshes, finalTex, this.opts.resolution, stats,
            { lightmapper, composite, refinement, positionTexture: atlas.positionTexture, normalTexture: atlas.normalTexture },
        );
    }
}
