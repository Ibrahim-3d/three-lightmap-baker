...
export class AtlasViewer {
    ...
    setTexture(tex: Texture | null): void { this.mat.uniforms.map.value = tex; }
    setSRGB(v: boolean): void              { this.mat.uniforms.sRGB.value = v; }
    setSize(px: number): void              { this.size = px; }
    setMargin(px: number): void            { this.margin = px; }
    setCorner(c: AtlasViewerCorner): void  { this.corner = c; }
    setCollapsed(v: boolean): void         { this.collapsed = v; this.refreshHeaderText(); }
    /** Updates the active-layer label shown in the header. */
    setLayerLabel(label: string): void     { this.layerLabel = label; this.refreshHeaderText(); }
    ...
    attachHeader(parent: HTMLElement = document.body): void {
        ...
        Object.assign(el.style, {
            ...
            color: '#ddd', backgroundColor: 'rgba(0,0,0,0.78)',
            padding: '4px 8px', cursor: 'pointer', userSelect: 'none',
            border: '1px solid #444', borderRadius: '3px',
            zIndex: '50', display: 'none', lineHeight: `${HEADER_HEIGHT - 10}px`,
    ...
    private positionHeader(canvasRect: DOMRect): void {
        if (!this.headerEl) return;
        if (!this.visible) { this.headerEl.style.display = 'none'; return; }
        this.headerEl.style.display = 'block';
        this.headerEl.style.width = `${this.size}px`;

        let left = 0, topBody = 0;
        switch (this.corner) {
            case 'tl': left = this.margin;                          topBody = this.margin + HEADER_HEIGHT; break;
            case 'tr': left = canvasRect.width - this.size - this.margin;  topBody = this.margin + HEADER_HEIGHT; break;
            case 'bl': left = this.margin;                          topBody = canvasRect.height - this.margin - this.size; break;
            case 'br': left = canvasRect.width - this.size - this.margin;  topBody = canvasRect.height - this.margin - this.size; break;
        }
    ...
    render(renderer: WebGLRenderer): void {
        ...
        let x = 0, y = 0;
        switch (this.corner) {
            case 'tl': x = m;          y = h - sz - m - Math.floor(HEADER_HEIGHT * dpr); break;
            case 'tr': x = w - sz - m; y = h - sz - m - Math.floor(HEADER_HEIGHT * dpr); break;
            case 'bl': x = m;          y = m;          break;
            case 'br': x = w - sz - m; y = m;          break;
        }
        ...
        renderer.setScissor(this.prevScissor.x, this.prevScissor.y, this.prevScissor.z, this.prevScissor.w);
        renderer.setViewport(this.prevViewport.x, this.prevViewport.y, this.prevViewport.z, this.prevViewport.w);
