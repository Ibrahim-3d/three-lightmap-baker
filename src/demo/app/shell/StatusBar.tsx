...
import { bakeProgress, bakeStatus, isStale, layout, logBuffer } from '../../state/signals';
  ...
  return (
    <footer class="h-10 bg-bg-1 border-t border-border flex items-center gap-3 px-3 text-[12px] pointer-events-auto relative">
      ...
      <div class="flex items-center gap-3 text-text-1 font-mono text-[11px]">
        ...
        {p.atlas > 0 && <span>Atlases: {p.atlas}</span>}
      ...
      {entries.length === 0 ? (
        ...
        <ul>
          {entries.map((e) => (
            ...
            >
              <span class="text-text-2 mr-2">{new Date(e.ts).toLocaleTimeString()}</span>
              {e.msg}
            </li>
          ))}
        </ul>
