...
export function Row(props: { label: string; children: JSX.Element | JSX.Element[]; hint?: string }) {
    return (
        <div class="flex items-center gap-2 px-2 h-6 hover:bg-bg-2">
            <label class="text-[11px] text-text-1 w-28 flex-shrink-0 truncate" title={props.hint ?? props.label}>
                {props.label}
            </label>
...
export function TextField(props: { value: string; onChange: (v: string) => void; placeholder?: string }) {
    return (
        <input
            type="text"
...
export function RangeField(props: {
    ...
    return (
        <>
            ...
            />
            <span class="text-[10px] text-text-2 w-10 text-right font-mono">{props.value.toFixed(2)}</span>
        </>
    );
}
...
export function BoolField(props: { value: boolean; onChange: (v: boolean) => void; label?: string }) {
    return (
