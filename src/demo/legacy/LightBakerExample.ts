import { Color, DirectionalLight, DoubleSide, LinearFilter, Mesh, MeshBasicMaterial, MeshStandardMaterial, NearestFilter, Object3D, PerspectiveCamera, PlaneGeometry, Scene, Texture, Vector3, WebGLMultipleRenderTargets, WebGLRenderer, SRGBColorSpace } from 'three';
...
export class LightBakerExample {
    ...
    lightmapTexture: WebGLMultipleRenderTargets;
    ...
    onDebugTexturesChange() {
        ...
        if(this.options.debugTextures) {
            ...
            this.debugLightmap = this.createDebugTexture(this.lightmapTexture.texture[0], new Vector3(24, 10, 0));
        }
    }
        ...
        this.currentModel.traverse((child: any) => {
            if(child.isMesh) {
                ...
                if(this.options.renderMode == "lightmap") {
                    child.material.lightMap = this.lightmapTexture.texture[0];
                    child.material.lightMap.channel = 2;
                }
